...
1.  **Internal Standard (IS) Method: This is the most core and effective method.**
    * **Stable Isotope-Labeled Internal Standard (Stable Isotope-Labeled IS):** This is the gold standard. It uses a compound chemically identical to the analyte but with some atoms replaced by stable isotopes (e.g., ²H, ¹³C, ¹⁵N) as the internal standard (e.g., using a ¹³C-labeled amino acid when detecting amino acids). Due to nearly identical chemical properties, the internal standard and analyte behave very similarly during sample preparation and ionization. Any factors causing fluctuations in ionization efficiency affect both the analyte and the internal standard equally. By calculating the ratio of **analyte peak area / IS peak area**, errors due to ionization efficiency fluctuations are greatly mitigated.
    * **Structural Analog Internal Standard (Structural Analog IS):** If an isotope-labeled IS is unavailable, a compound with a structure very similar to the analyte is chosen as the IS, but its correction capability is not as perfect as the isotope-labeled IS.

2.  **Calibration Curve Method:**
    * Prepare a series of standard solutions with known concentrations of the standard analyte, spiked with a **fixed amount of internal standard**.
    * After instrumental analysis, plot a calibration curve (typically a linear regression curve) with the peak area ratio of analyte to IS on the y-axis and the standard concentration on the x-axis.
    * The concentration of an unknown sample is calculated using this curve, enabling quantification.

3.  **Quality Control (QC) Samples:**
    * QC samples, prepared from pooled samples, are periodically injected throughout the analytical batch. By monitoring the stability and precision of the QC results, the reproducibility of the entire experimental process and the reliability of the data can be assessed.

**Summary:** The combination of **stable isotope-labeled internal standards + calibration curve method** maximally corrects for errors arising from differences in ionization efficiency, sample preparation losses, and instrumental fluctuations, thereby ensuring quantitative accuracy.
...
* **Proteomics:**
  * **Primary Ion Source: Electrospray Ionization (ESI).** Especially nano-electrospray (nano-ESI), coupled with High-Performance Liquid Chromatography (particularly nano-flow LC, nLC), is the absolute mainstream technology for "bottom-up" proteomics (analysis of enzymatically digested peptides), known for its high sensitivity.
  * **Important Ion Source: Matrix-Assisted Laser Desorption/Ionization (MALDI).** Commonly used for "Peptide Mass Fingerprinting" identification and Imaging Mass Spectrometry (MSI). It is characterized by simple operation, speed, and good salt tolerance, typically coupled with Time-of-Flight mass spectrometers (TOF) as MALDI-TOF/TOF.
...
* **Hard Ionization Source:**
  ...
  * **Advantage:** Fragment ions provide rich structural information, useful for structural elucidation of unknowns and library searching.
  * **Representative: Electron Ionization (EI) Source.** It is the standard ion source for GC-MS.
...
* **Soft Ionization Source:**
  ...
  * **Advantage:** Very suitable for analyzing complex mixtures, large molecules (e.g., proteins, nucleic acids), and thermally labile compounds, allowing direct acquisition of accurate molecular weight information.
  * **Disadvantage:** Lacks structural information, often requiring Tandem Mass Spectrometry (MS/MS) to induce fragmentation for structural insights.
...
For sources like ESI that can produce multiply charged ions, directly determining the charge state (z) of an ion is key to determining its molecular weight (M). There are two main methods:
...
1.  **Isotopic Peak Distribution Method (The most direct method)**
    ...
    * **Procedure:** Zoom in on a peak cluster in a high-mass resolution mass spectrum (e.g., from Q-TOF, Orbitrap, FT-ICR).
...
2.  **Charge Deconvolution Method**
    ...
    * **Procedure:** Modern mass spectrometer software includes automatic "deconvolution" functions that can automatically identify multiple charge state peaks belonging to the same molecule and directly output its single-charge molecular weight M.
